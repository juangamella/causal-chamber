...
| scm_1_reference | [`generators/scms.py`](lt_camera_v1/generators/scms.py)| The camera parameters are fixed to $\text{Ap} = 1.8, \text{ISO} = 500, T_\text{Im}=0.005$. We sample $Z := (R, G, B, \theta_1, \theta_2)^T$ following a structural causal model, given by $Z := S\tilde{Z} + l with $S := \mathrm{diag}(255, 255, 255, 180, 180)$, $l:=(0,0,0,-90,-90)^T$ and $\tilde{Z} := WZ + D\epsilon$, where $W$ is upper triangular up to a permutation of rows and columns, $D$ is a diagonal matrix with positive entries, and $\epsilon_i \overset{\text{i.i.d.}}{\sim} \mathrm{Unif}[0,1]$ and $\epsilon_i \perp\!\!\!\!\perp \epsilon_j \forall i,j$. $\sum_{i=1}^5 (W+D)_{j,i} = 1$ for all $j$, i.e., $W+D$ has rows that sum to 1. The values of $W$ and $D$ are given in [`scm_1.py`](lt_camera_v1/scm_1.py).  |
