...
|   ar\_1\_uniform\_ref | [`generators/actuators_ar.py`](generators/actuators_ar.py) | We sample the values of $R,G,B,\theta_1,\theta_2$ from scaled and shifterd stationary $\text{AR}(1)$ processes as follows. For each actuator, we sample $N=3\times10^{4}$ observations from the $\text{AR}(1)$ process $X(t) := \phi X(t-1) + \epsilon_t$ where $\epsilon_1,\ldots,\epsilon_N \overset{\text{i.i.d.}}{\sim} \text{Unif}[-1,1]$ and $\phi = 0.5, 0.6, 0.7, 0.8, 0.9$ for $R,G,B,\theta_1,\theta_2$, respectively. For each actuator, we then scale and shift $X(t)$ so that its values fall in the range $[\mu - \Delta, \mu + \Delta]$, where $\mu,\Delta := (128,120)$ for $R,G,B$ and $\mu,\Delta := (0,20)$ for $\theta_1, \theta_2$. |
